...
class DetailAdapter(private val list:ArrayList<MainData>):RecyclerView.Adapter<DetailAdapter.ViewHolder>() {
    //아이템의 갯수
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): DetailAdapter.ViewHolder {
        val v = LayoutInflater.from(parent.context).inflate(R.layout.activity_detail_item, parent, false)
        return ViewHolder(v)
    }
    ...
    class ViewHolder(view: View):RecyclerView.ViewHolder(view) {
        fun bindItems(data : MainData){
            ...
            itemView.setOnClickListener({
                Toast.makeText(itemView.context, "아이템 '${data.name}'를 클릭했습니다.", Toast.LENGTH_LONG)
                    .show()
            })
        }
    }
